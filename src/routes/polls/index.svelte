<script lang="ts">
  ...
  import { getPolls, userVote } from "@/lib/dmart_services/dmart_services";
  import {
    errorToastMessage,
    successToastMessage,
      ...
      if (response?.status === "success" && response?.records) {
        const processedPolls = response.records.map((poll) => {
          ...
          return {
            ...
            totalVotes,
            createdBy:
              poll.attributes?.owner_shortname || $_("polls.unknown_author"),
  ...
  function formatTimeAgo(date) {
    ...
    for (const [unit, secondsInUnit] of Object.entries(intervals)) {
      ...
      if (interval >= 1) {
        return `${interval}${unit} ${$_("polls.time_ago")}`; // like 5m ago, 2h ago
      }
    }
    return $_("polls.just_now");
  }
</script>
...
<div class="polls-container min-h-screen bg-white" class:rtl={$isRTL}>
  <!-- Header -->
  <div
    class="polls-header flex justify-between items-center mb-10 pt-10 px-8 max-w-7xl mx-auto"
  >
    <div class="header-text">
      <h1 class="text-3xl font-bold text-gray-900 mb-2">
        {$_("polls.page_title")}
      </h1>
      <p class="text-gray-500 text-sm">{$_("polls.page_subtitle")}</p>
    </div>
    <button
      class="bg-indigo-600 hover:bg-indigo-700 text-white px-5 py-2 rounded-full font-medium flex items-center gap-2 transition-colors shadow-sm text-sm  cursor-pointer"
  ...
  <div class="max-w-7xl mx-auto border-t border-gray-100 mb-8 pt-8 px-8">
    ...
    <div class="polls-stats flex gap-4 mb-10">
      <div
        class="stat-pill flex items-center gap-2 px-5 py-2 bg-white border border-gray-100 rounded-full text-sm shadow-sm ring-1 ring-black ring-opacity-5"
      >
        <ChartOutline class="w-4 h-4 text-indigo-500" />
        <span class="text-gray-500">{$_("polls.stat_total")}</span>
        <span class="font-bold text-gray-900 ml-1">{polls.length}</span>
      </div>
      <div
        class="stat-pill flex items-center gap-2 px-5 py-2 bg-white border border-gray-100 rounded-full text-sm shadow-sm ring-1 ring-black ring-opacity-5"
      >
        <ClockOutline class="w-4 h-4 text-green-500" />
        <span class="text-gray-500">{$_("polls.stat_active")}</span>
        <span class="font-bold text-gray-900 ml-1"
          >{polls.filter((p) => p.isActive).length}</span
        >
      </div>
      <div
        class="stat-pill flex items-center gap-2 px-5 py-2 bg-white border border-gray-100 rounded-full text-sm shadow-sm ring-1 ring-black ring-opacity-5"
      >
        ...
        <span class="text-gray-500">{$_("polls.stat_ended")}</span>
        <span class="font-bold text-gray-900 ml-1"
          >{polls.filter((p) => !p.isActive).length}</span
        >
      </div>
    ...
    <!-- Controls Row -->
    <div
      class="polls-controls flex flex-col sm:flex-row gap-6 mb-8 items-center"
    >
      ...
      <div
        class="filter-pills flex items-center bg-white border border-gray-200 rounded-full p-1 shadow-sm h-10"
      >
        <button
          class="px-6 h-full rounded-full text-sm font-medium transition-colors cursor-pointer {filterStatus ===
          'all'
    ...
    <!-- Polls Grid -->
    <div class="polls-content min-h-[400px]">
      {#if loading}
      ...
      {:else if filteredPolls.length === 0}
        <div
          class="empty-state flex flex-col items-center justify-center py-16 text-gray-500"
        >
      ...
      {:else}
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 pb-20">
          {#each filteredPolls as poll}
            <div
              class="bg-white rounded-[1.5rem] p-8 border border-gray-100 shadow-sm hover:shadow-md transition-shadow relative ring-1 ring-black ring-opacity-5"
            >
              <!-- Card Top -->
              <div class="flex justify-between items-start mb-6">
                <div class="flex items-center gap-3">
                  <div
                    class="w-10 h-10 rounded-full bg-gray-100 flex items-center justify-center text-gray-500 font-semibold text-xs border border-gray-200"
                  >
                ...
                <div
                  class="px-3 py-1 rounded-full text-[11px] font-bold tracking-wide uppercase {poll.isActive
                    ? 'bg-green-50 text-green-600'
                    : 'bg-gray-100 text-gray-500'}"
                >
                  {poll.isActive
                    ? $_("polls.status_active")
                    : $_("polls.status_ended")}
                </div>
              </div>
              ...
              <p
                class="text-[13px] text-gray-500 mb-8 line-clamp-2 leading-relaxed"
              >
                {poll.description}
              </p>
              ...
              <div class="mb-8">
                {#if poll.candidates.length > 0}
                  {@const sorted = [...poll.candidates].sort(
                    (a, b) => b.votes - a.votes,
                  )}
                  ...
                  <div class="flex justify-between items-center text-xs mb-2.5">
                    <span class="text-gray-500 font-medium">
                      {$_("polls.leading")}:
                      <span class="text-gray-700">{leading.name}</span>
                    </span>
                    <span class="font-bold text-indigo-600"
                      >{leading.percentage}%</span
                    >
                  </div>
                  <div
                    class="w-full h-[6px] bg-gray-100 rounded-full overflow-hidden"
                  >
                    <div
                      class="h-full bg-indigo-500 rounded-full transition-all duration-500"
                      style="width: {leading.percentage}%"
                    ></div>
                  </div>
                {/if}
              </div>
              ...
              <div
                class="flex items-center justify-between pt-5 border-t border-gray-100 mt-auto"
              >
                <div
                  class="flex items-center gap-4 text-[11px] font-medium text-gray-400"
                >
                  <span class="flex items-center gap-1.5">
                    <UserOutline class="w-3.5 h-3.5" />
                    {poll.totalVotes}
                    {$_("polls.votes")}
                  </span>
                  <span>{poll.candidates.length} {$_("polls.options")}</span>
                  <span
                    >{poll.tags.length > 0
                      ? poll.tags.join(", ")
                      : $_("polls.default_tag")}</span
                  >
                </div>

                <div class="flex items-center gap-3">
                  <button
                    class="flex items-center gap-1.5 px-4 py-1.5 rounded-full bg-gray-50 hover:bg-gray-100 text-gray-600 border border-gray-200 text-xs font-semibold transition-colors"
                    onclick={() => openResultsModal(poll)}
                  >
                    <EyeOutline class="w-[14px] h-[14px]" />
                    {$_("polls.results")}
                  </button>

                  {#if poll.hasVoted}
                    <button
                      class="flex items-center gap-1.5 px-4 py-1.5 rounded-full bg-green-50 text-green-600 border border-green-100 text-xs font-semibold transition-colors cursor-default"
                    >
                      <CheckCircleOutline class="w-[14px] h-[14px]" />
                      {$_("polls.voted")}
                    </button>
                  {:else if poll.isActive}
                    <button
                      class="flex items-center gap-1.5 px-5 py-1.5 rounded-full bg-indigo-600 hover:bg-indigo-700 text-white shadow-sm shadow-indigo-200 text-xs font-semibold transition-colors"
...
{#if showVoteModal && selectedPoll}
  <div
    class="fixed inset-0 bg-gray-900/60 flex items-center justify-center z-50 p-4 backdrop-blur-sm"
    onclick={closeModal}
  ...
  >
    <div
      class="bg-white rounded-3xl w-full max-w-lg max-h-[90vh] flex flex-col shadow-2xl overflow-hidden"
    ...
    >
      ...
      <div class="p-8 overflow-y-auto">
            ...
            {#each selectedPoll.candidates as candidate}
              <div>
                <div class="flex justify-between items-end mb-2">
                  <span class="text-[13px] text-gray-800 font-semibold"
                    >{candidate.name}</span
                  >
        ...
        {:else if selectedPoll.isActive && !selectedPoll.hasVoted}
          <div class="space-y-3">
            {#each selectedPoll.candidates as candidate}
              <button
                class="w-full flex items-center gap-4 p-5 rounded-2xl border-2 transition-all text-left {selectedCandidate ===
                candidate.key
        ...
        {:else}
          <div class="text-center pb-6 border-b border-gray-100 mb-6">
            {#if selectedPoll.hasVoted}
              <div
                class="inline-flex items-center justify-center gap-2 bg-green-50 text-green-700 px-5 py-2.5 rounded-full font-semibold text-sm"
              >
                <CheckCircleOutline class="w-4 h-4" />
                {$_("polls.you_voted_for")}
                <span class="text-green-800">{selectedPoll.userVote}</span>
              </div>
            {:else}
              <div
                class="inline-flex items-center justify-center gap-2 bg-gray-100 text-gray-600 px-5 py-2.5 rounded-full font-semibold text-sm"
              >
      ...
      <!-- Modal Footer -->
      <div
        class="flex justify-end gap-3 p-6 border-t border-gray-100 bg-gray-50/80"
      >
        <button
          class="px-6 py-2.5 rounded-full text-sm font-semibold text-gray-600 hover:bg-gray-200 transition-colors"
          onclick={closeModal}
        >
          {$_("polls.cancel")}
        </button>
        {#if !showResults && selectedPoll.isActive && !selectedPoll.hasVoted}
          <button
            class="px-8 py-2.5 rounded-full text-sm font-semibold text-white bg-indigo-600 hover:bg-indigo-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2 shadow-sm shadow-indigo-200"
            onclick={submitVote}
            disabled={!selectedCandidate || votingInProgress}
          >
            {#if votingInProgress}
              <div
                class="w-4 h-4 border-[2px] border-white/30 border-t-white rounded-full animate-spin"
              ></div>
            {/if}
            {$_("polls.vote")}
          </button>
